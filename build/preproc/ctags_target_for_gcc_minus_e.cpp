...
int key[4]; // 키패드에서 입력받는 번호 저장
int password[4] = {1, 2, 3, 4}; // 비밀번호 설정
...
void loop() {
  int a = getnumber(); // 키 패드에서 숫자 입력 받기
    ...
    if (a != 0){ // 키패드에서 입력 받는 숫자가 0이 아닐 때
      ...
      if(count == 4){ // 입력한 숫자의 개수가 비밀번호 개수와 같을 경우

        for(int j=0; j<4; j++){ // 입력한 숫자와 비밀번호의 숫자 비교
          if(password[j] == key[j])
...
byte getnumber(void){
  ...
  for(count = 1; count <= 8; count++) { // 번호를 1~16까지 비교해줌
    digitalWrite(4, 0x0); // 동기용 클럭 신호 low(Serial Clock)
    if (digitalRead(5) == 0) // 입력 받는 수와 1~16까지 비교하는 숫자가 맞을경우
